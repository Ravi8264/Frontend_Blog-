...
const Addpost = () => {
  ...
  const { postId } = useParams(); // Get postId from URL params for editing
  ...
  const [post, setPost] = useState(null); // For editing mode
  ...
  }, [handleAuthError, isEditMode, postId, navigate]);
  ...
  const validateForm = () => {
    ...
    // Validate image based on mode (only for create mode)
    if (!isEditMode) {
      if (imageMode === "link") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        // Create post
        await createPost(postData, currentUser.id, formData.categoryId);
    ...
    return (
      <div className="addpost-container">
        <div className="loading">
          {isEditMode ? "Loading post data..." : "Loading required data..."}
        </div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit}>
        ...
        {currentUser && (
          <div className="user-info">
            <p>
              {isEditMode ? "Editing" : "Creating"} post as:{" "}
          ...
          <div className="form-group">
            ...
            {imageMode === "link" && (
              <div className="image-link-input">
                ...
                />
                <small>
                  Please provide a valid image URL. If no image is provided, a
